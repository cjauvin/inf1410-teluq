...
### Les problèmes, et la manière dont vous les avez réglés

Un projet informatique d'une certaine envergure ne se déroule à peu près jamais sans accroc. Une fonction qui renvoie le mauvais résultat dans un cas qu'on n'avait pas prévu, un import qui échoue parce que l'éditeur ne regarde pas dans le bon environnement, une branche qu'on fusionne et qui entre en conflit avec le travail d'un coéquipier, une dépendance mise à jour qui casse un test qui passait la veille&nbsp;: ce sont des événements ordinaires du développement, pas des accidents de parcours. Une bonne partie des outils présentés dans ce cours existent précisément pour eux. Les [tests]({{< relref "module2/20-tests/index.md" >}}) détectent le problème, le [débogueur]({{< relref "module2/25-debogage/index.md" >}}) le localise, et [git]({{< relref "module2/30-versioning/index.md" >}}) permet de revenir en arrière, de comparer deux états du code, et de réparer un historique qu'on a emmêlé. Savoir se servir de ces outils, c'est d'abord savoir traiter les problèmes quand ils se présentent, et ils se présenteront.

C'est pourquoi les problèmes que vous rencontrez, et la manière dont vous les avez réglés, font partie de la documentation attendue, au même titre que les décisions d'architecture. Pas une ligne dans un bilan de fin de session, mais un récit détaillé, dans le blogue, écrit à chaud&nbsp;: quel était le symptôme, comment vous avez cerné la cause, quel outil vous a permis de la trouver, ce que vous avez changé, et ce que vous en avez retenu. Deux familles de problèmes sont attendues en particulier. D'abord les problèmes de logique et de programmation, un calcul faux, un cas limite oublié, un comportement qui ne correspond pas à ce que le code semble dire, résolus par une technique de débogage, qu'il s'agisse d'un `print` bien placé, d'un point d'arrêt ou de la lecture de la pile d'appels. Ensuite les problèmes de gestion du code source, un conflit de fusion, un commit fait sur la mauvaise branche, un fichier qu'il faut ressusciter d'une version antérieure, résolus avec git. Le bon réflexe, quand un problème apparaît, est d'ouvrir une [issue GitHub]({{< relref "module4/10-github/index.md" >}}) qui le décrit, le symptôme et la façon de le reproduire, avant même d'en chercher la cause, puis de la fermer par le commit ou la pull request qui le règle, en citant son numéro dans le message. L'issue, le commit et le billet de blogue se renvoient alors les uns aux autres, et la [trace du temps](#la-trace-du-temps) s'écrit d'elle-même.

Il reste toujours possible, miraculeusement, qu'aucun problème ne survienne pendant toute la conception d'un projet. Mais c'est si rare que, a priori, une telle absence paraîtrait un peu suspecte. On s'attend plutôt à ce qu'un projet en compte plusieurs, et à ce qu'ils soient racontés&nbsp;: un projet dont le journal ne mentionne aucune difficulté n'a probablement pas tout dit.
...
Il est possible, et même souhaitable, de présenter une partie de vos travaux sous la forme de vidéos, idéalement sur YouTube. La raison tient à ce qui précède. Tout ce que vous remettez peut, en principe, avoir été produit par délégation&nbsp;: du code, un ADR, une entrée de blogue, même une transcription. Une vidéo où l'on vous voit, ou du moins où l'on vous entend, faire fonctionner votre application, parcourir votre code et expliquer ce que vous avez fait, reste en bout de ligne le type d'artéfact le plus difficile à automatiser. Elle se rapproche de l'entretien de suivi, avec l'avantage que vous choisissez le moment et ce que vous montrez. C'est pourquoi elle est généralement vue d'un bon oeil, et qu'elle compte, dans l'évaluation, parmi les preuves les plus convaincantes d'un travail réellement fait.

Une vidéo convient à tout ce qui est de l'ordre de la démonstration et du récit&nbsp;: faire tourner l'application devant la caméra, parcourir le code en expliquant ses choix, raconter un bogue et la manière dont vous l'avez trouvé, ou tenir une entrée de blogue à voix haute plutôt qu'à l'écrit. Elle ne remplace pas ce qui doit vivre dans le dépôt sous forme de texte, l'historique git, les ADRs, les issues, parce que ces artéfacts sont faits pour être lus, cherchés et comparés, et qu'ils portent leur propre trace du temps. La vidéo vient s'y ajouter, et elle est d'autant plus convaincante qu'elle renvoie à eux, en montrant à l'écran le commit, la pull request ou l'issue dont elle parle.

Sur le plan pratique, rien de compliqué. Un enregistrement de l'écran avec votre voix suffit, fait avec l'outil de votre système ou avec [OBS Studio](https://obsproject.com), qui est libre et gratuit. Quelques minutes valent mieux qu'une heure&nbsp;: une démonstration de cinq à dix minutes, qui va droit à ce qu'elle veut montrer, se regarde jusqu'au bout. Aucune qualité de production n'est attendue, ni montage, ni générique, et les hésitations font partie du genre. Déposez la vidéo sur YouTube en mode *non répertorié*, ce qui la rend accessible à quiconque a le lien sans la rendre publique, et placez ce lien dans l'entrée de blogue ou le `README.md` qui s'y rapporte, avec la date de l'enregistrement.
...
- Un **problème de logique ou de programmation** rencontré pendant le développement, et raconté en détail dans le blogue, comme le demande la section sur [les problèmes](#les-problèmes-et-la-manière-dont-vous-les-avez-réglés)&nbsp;: le symptôme, la manière dont vous l'avez localisé, par exemple avec un point d'arrêt, le panneau des variables et la pile d'appels de l'éditeur, comme dans la section sur [le débogage]({{< relref "module2/25-debogage/index.md" >}}), et ce que vous avez compris en le trouvant. Une transcription de session peut compléter le billet, et des captures d'écran sont préférables. Un bogue bien raconté vaut souvent plus qu'une fonctionnalité de plus.
- Un **problème de gestion du code source** réglé avec [git]({{< relref "module2/30-versioning/index.md" >}}), raconté de la même manière&nbsp;: un conflit de fusion, un commit fait sur la mauvaise branche, une modification à retrouver dans l'historique, ou un dépôt à remettre d'aplomb. Les commandes exactes, ou les gestes faits dans l'éditeur, font partie du récit, avec l'état du dépôt avant et après.
...
- Un **bilan des problèmes** de toute la session, dans le blogue&nbsp;: la liste de ceux que vous avez rencontrés depuis le jalon 1, chacun renvoyant au billet qui le raconte, avec un mot sur ceux qui vous ont coûté le plus de temps, et sur ce que vous feriez aujourd'hui pour les éviter, ou pour les trouver plus vite.
