...
Au-delà des différences d'approche entre ces frameworks, certains problèmes reviennent systématiquement dès qu'on construit une application web non triviale. Le **state management** (gestion de l'état) est le plus fondamental : quand plusieurs composants doivent partager des données (l'utilisateur connecté, le contenu d'un panier d'achat, les préférences d'affichage), il faut un mécanisme pour centraliser cet état et le propager aux composants concernés. Chaque framework a développé ses propres solutions : Redux puis Zustand pour React, Vuex puis Pinia pour Vue, les stores natifs pour Svelte. Le **routing côté client** est l'autre problème récurrent : dans une SPA, la navigation entre les "pages" est gérée par le JavaScript plutôt que par le serveur, ce qui nécessite un routeur qui associe des URLs à des composants et qui synchronise la barre d'adresse du navigateur avec l'état de l'application. Ces préoccupations transversales expliquent pourquoi les meta-frameworks (Next.js, Nuxt, SvelteKit) qu'on a mentionnés dans la section sur les architectures web ont gagné en popularité : ils intègrent le routing, le rendu serveur et la gestion des données dans un cadre cohérent, évitant au développeur d'assembler ces pièces lui-même.
