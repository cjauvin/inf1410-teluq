...
Pour lancer ce serveur, il faut comprendre que FastAPI et Uvicorn jouent des rôles complémentaires. FastAPI est un *framework* : il fournit les outils pour définir des endpoints, valider les données et générer la documentation. Mais FastAPI ne sait pas, par lui-même, écouter sur un port réseau et recevoir des requêtes HTTP. C'est le rôle d'Uvicorn, un *serveur ASGI* (Asynchronous Server Gateway Interface) : il écoute les connexions entrantes, reçoit les requêtes HTTP, les transmet à FastAPI pour traitement, puis renvoie les réponses au client. L'analogie serait celle d'un restaurant, une autre cuisine que celle de la section sur la concurrence : Uvicorn est le maître d'hôtel qui accueille les clients et prend les commandes, FastAPI est la cuisine qui prépare les plats. Le mot *asynchrone* dans ASGI n'est pas décoratif&nbsp;: Uvicorn fait tourner une boucle d'événements, et une fonction déclarée avec `async def` lui rend la main pendant qu'elle attend, ce que la section sur [la concurrence]({{< relref "/docs/module2/15-concurrence/50-ne-jamais-bloquer" >}}) explique en détail. On peut utiliser `uv` pour installer les deux dépendances et lancer le tout sans configuration préalable :
